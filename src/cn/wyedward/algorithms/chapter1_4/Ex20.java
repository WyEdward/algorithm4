...
public class Ex20 {
     ...
     * @return
     */
    public static int drank(int key, int[] a, int lo, int hi){
        if(lo > hi){
            return -1;
        }
        int mid = lo + (hi - lo) / 2;
        if(key < a[mid]){
            return drank(key, a, mid + 1, hi);
        }else if(key > a[mid]){
            return drank(key, a, lo, mid - 1);
        }else{
            return mid;
        }
    }
     ...
     * @return
     */
    public static int irank(int key, int[] a, int lo, int hi){
        if(lo > hi){
            return -1;
        }
        int mid = lo + (hi - lo) / 2;
        if(key > a[mid]){
            return irank(key, a, mid + 1, hi);
        }else if(key < a[mid]){
            return irank(key, a, lo, mid - 1);
        }else{
            return mid;
        }
    }
    public static boolean contains(int[] a,int key){
        int max = max(a);
        if(irank(key, a, 0, max) != -1){
            return true;
        }else if(irank(key, a, max+1, a.length - 1) != -1){
            return true;
        }else{
            return false;
        }
